...
class DatabaseManager {
  ...
  dbPath;
  constructor(dbPath) {
    this.dbPath = dbPath || path.join(electron.app.getPath("userData"), "mothercore.db");
    ...
    try {
      const transaction = this.db.transaction(() => {
        const stmt = this.db.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?)
        `);
        const result2 = stmt.run(id, name, description, color, icon);
        console.log("Organization created in database with result:", result2);
    ...
    try {
      const transaction = this.db.transaction(() => {
        ...
        const stmt = this.db.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?)
        `);
        const result2 = stmt.run(id, organization_id, name, description, color);
        console.log("Project created in database with result:", result2);
  ...
  getProjects(organizationId) {
    ...
    try {
      ...
      console.log(
        ...
        projects.map((p) => ({ id: p.id, name: p.name, org_id: p.organization_id }))
      );
      return projects;
    } catch (error) {
    ...
    try {
      const transaction = this.db.transaction(() => {
        ...
        const stmt = this.db.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `);
        const result2 = stmt.run(id, project_id, name, description, cover_image, spine_color, position);
        console.log("Book created in database with result:", result2);
  ...
  getBooks(projectId) {
    ...
    try {
      ...
      console.log(
        ...
        books.map((b) => ({ id: b.id, name: b.name, project_id: b.project_id }))
      );
      return books;
    } catch (error) {
    ...
    try {
      const transaction = this.db.transaction(() => {
        ...
        const stmt = this.db.prepare(`
          ...
          VALUES (?, ?, ?, ?)
        `);
        const result2 = stmt.run(id, book_id, name, position);
        console.log("Chapter created in database with result:", result2);
  ...
  getChapters(bookId) {
    ...
    try {
      ...
      console.log(
        ...
        chapters.map((c) => ({ id: c.id, name: c.name, book_id: c.book_id }))
      );
      return chapters;
    } catch (error) {
    ...
    try {
      const transaction = this.db.transaction(() => {
        ...
        const stmt = this.db.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `);
        const result2 = stmt.run(id, chapter_id, title, contentStr, content_text, page_type, tags, position);
        console.log("Page created in database with result:", result2);
  ...
  getPages(chapterId) {
    ...
    try {
      ...
      console.log(
        ...
        pages.map((p) => ({ id: p.id, title: p.title, chapter_id: p.chapter_id }))
      );
      return pages;
    } catch (error) {
    ...
    try {
      const transaction = this.db.transaction(() => {
        ...
        const stmt = this.db.prepare(`
          ...
          WHERE id = ?
        `);
        const result2 = stmt.run(contentStr, plainText, id);
        console.log(`Page content updated with result:`, result2);
        return result2;
      });
      const result = transaction();
      return result;
  ...
  logDbState() {
    console.log("\n==== DATABASE STATE ====");
    try {
      const orgs = this.db.prepare("SELECT * FROM organizations").all();
...
Chapters (${chapters.length}):`);
      ...
      const pages = this.db.prepare("SELECT * FROM pages").all();
      console.log(`
Pages (${pages.length}):`);
      pages.forEach((page) => {
...
class SecureUpdateManager {
  ...
  async ensureDirectoriesExist() {
    try {
      await fs$1.mkdir(this.tempDir, { recursive: true });
      await fs$1.mkdir(this.backupDir, { recursive: true });
      await fs$1.mkdir(this.updateDir, { recursive: true });
    } catch (error) {
  ...
  loadSettings() {
    ...
    const settings = {
      ...
      updateSource: settingsGroup.updateSource || "manual",
  ...
  async downloadUpdate(updateInfo) {
    try {
      ...
      if (updateInfo.url.startsWith("http")) {
        ...
        const fileStream = await fs$1.open(downloadPath, "w");
        const buffer = await response.buffer();
        await fileStream.write(buffer);
        await fileStream.close();
      } else {
        await fs$1.copyFile(updateInfo.url, downloadPath);
      }
      const isValid = await this.verifyUpdatePackage(downloadPath, updateInfo);
      if (!isValid) {
        await fs$1.unlink(downloadPath);
   ...
   */
  async installUpdate(updatePath, userApproved) {
   ...
   */
  async verifyLocalUpdate(filePath) {
    try {
   ...
   */
  async checkRemoteUpdate(domain) {
    try {
   ...
   */
  async verifyUpdatePackage(packagePath, updateInfo) {
    try {
      const fileData = await fs$1.readFile(packagePath);
      const calculatedChecksum = require$$0$1.createHash("sha256").update(fileData).digest("hex");
   ...
   */
  meetsMinimumVersion(requiredVersion) {
    const current = this.appVersion.split(".").map(Number);
...
function createWindow() {
  ...
  if (!electron.app.isPackaged && process.env["VITE_DEV_SERVER_URL"]) {
...
function setupIpcHandlers() {
  electron.ipcMain.handle("check-auth-status", async () => {
    try {
  ...
  electron.ipcMain.handle("get-update-settings", async () => {
    try {
  ...
  electron.ipcMain.handle("save-update-settings", async (_, settings) => {
    try {
  ...
  electron.ipcMain.handle("check-for-updates", async (_, userRequested) => {
    try {
  ...
  electron.ipcMain.handle("download-update", async (_, updateInfo) => {
    try {
  ...
  electron.ipcMain.handle("install-update", async (_, userApproved) => {
    try {
  ...
  electron.ipcMain.handle("get-projects", async (_, organizationId) => {
    try {
  ...
  electron.ipcMain.handle("update-page-content", async (_, pageId, content, contentText) => {
    try {
  ...
  electron.ipcMain.handle("log-error", (_, errorMessage) => {
